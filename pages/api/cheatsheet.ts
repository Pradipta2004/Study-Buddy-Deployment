    ...
    try {
      ...
      if (isRateLimit) {
        ...
        if (attempt === maxRetries) {
          lastError = new Error('AI service rate limit reached. Please wait 1-2 minutes and try again. Larger PDFs may need more time between attempts.');
        }
        continue;
      }
      if (isServerError) {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const retryConfig = fileSizeMBNum > 5
      ? { maxRetries: 5, baseDelay: 20000, label: 'cheatsheet-generation (large PDF)' }
      : fileSizeMBNum > 2
      ? { maxRetries: 4, baseDelay: 15000, label: 'cheatsheet-generation (medium PDF)' }
      : { maxRetries: 3, baseDelay: 10000, label: 'cheatsheet-generation' };
    ...
    const userMessage = isRateLimit
      ? 'AI service rate limit reached. Please wait 1-2 minutes and try again. Larger PDFs may need more time between attempts.'
      : (msg || 'Failed to generate cheatsheet');
